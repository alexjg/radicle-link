...
use std::{
    collections::HashMap,
    env,
    os::unix::{io::RawFd, net::UnixListener, prelude::FromRawFd}, net::TcpListener,
};
...
///
/// This will check for the environment variables LISTEN_FDNAMES and LISTEN_FDS, which are set by
/// systemd. If neither of these environment variables are set this will return `None`, otherwise
/// it will load each socket and return a `HashMap` from the LISTEN_FDNAMES entry to the LISTEN_FDS
/// entry. This is similar to the behavior of `sd_listen_fds_with_names()`[1]
///
/// This function will return an error if 
/// * LISTEN_PID is not an integer
/// * LISTEN_FDS is not an integer
/// * The number of sockets indicated by LISTEN_FDS is not the same as the number of names in
///   LISTEN_FDNAMES
...
pub fn env_sockets() -> Result<Option<HashMap<String, Socket>>, Error> {
    let labelled_sockets = listen_fds::listen_fds()?.zip_longest(fd_names()).try_fold(
        ...
        |mut acc, item| {
            ...
            match item {
                Both(fd, name) => {
                    acc.insert(name.clone(), load_socket(fd)?);
                    Ok(acc)
                },
