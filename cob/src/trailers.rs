...
pub mod error {
    ...
    #[derive(Debug, Error)]
    pub enum InvalidContainingIdentityTrailer {
        #[error("no X-Rad-Containing-Identity trailer")]
        NoTrailer,
        #[error("no value for X-Rad-Containing-Identity")]
        NoValue,
        #[error("invalid git OID")]
        InvalidOid,
    }
...
impl From<ext::Oid> for SchemaCommitTrailer {
    fn from(oid: ext::Oid) -> Self {
        SchemaCommitTrailer(oid.into())
    }
}
...
impl TryFrom<Vec<OwnedTrailer>> for SchemaCommitTrailer {
    ...
    fn try_from(trailers: Vec<OwnedTrailer>) -> Result<Self, Self::Error> {
        SchemaCommitTrailer::try_from(&trailers)
    }
}
...
const CONTAINING_IDENTITY_TRAILER_TOKEN: &str = "X-Rad-Containing-Identity";

pub struct ContainingIdentityCommitTrailer(git2::Oid);

impl ContainingIdentityCommitTrailer {
    pub fn oid(&self) -> git2::Oid {
        self.0
    }
    ...
    pub fn from_trailers<'b, A, I>(
        trailers: I,
    ) -> Result<ContainingIdentityCommitTrailer, error::InvalidContainingIdentityTrailer>
    where
        A: Into<Trailer<'b>>,
        I: IntoIterator<Item = A>,
    {
        trailers
            ...
            .find_map(|trailer| {
                let trailer = trailer.into();
                if trailer.token == Token::try_from(CONTAINING_IDENTITY_TRAILER_TOKEN).unwrap() {
                    Some(ContainingIdentityCommitTrailer::try_from(&trailer))
                } else {
                    None
                }
            })
            .unwrap_or(Err(error::InvalidContainingIdentityTrailer::NoTrailer))
    }
}

impl From<git2::Oid> for ContainingIdentityCommitTrailer {
    fn from(oid: git2::Oid) -> Self {
        ContainingIdentityCommitTrailer(oid)
    }
}

impl From<ContainingIdentityCommitTrailer> for Trailer<'_> {
    fn from(containing: ContainingIdentityCommitTrailer) -> Self {
        Trailer {
            token: Token::try_from(CONTAINING_IDENTITY_TRAILER_TOKEN).unwrap(),
            values: vec![containing.0.to_string().into()],
        }
    }
}

impl TryFrom<Vec<Trailer<'_>>> for ContainingIdentityCommitTrailer {
    type Error = error::InvalidContainingIdentityTrailer;

    fn try_from(trailers: Vec<Trailer<'_>>) -> Result<Self, Self::Error> {
        ContainingIdentityCommitTrailer::from_trailers(trailers)
    }
}

impl TryFrom<&Trailer<'_>> for ContainingIdentityCommitTrailer {
    type Error = error::InvalidContainingIdentityTrailer;

    fn try_from(Trailer { values, .. }: &Trailer<'_>) -> Result<Self, Self::Error> {
        let val = values
            .first()
            .ok_or(error::InvalidContainingIdentityTrailer::NoValue)?;
        let oid = git2::Oid::from_str(val)
            .map_err(|_| error::InvalidContainingIdentityTrailer::InvalidOid)?;
        Ok(ContainingIdentityCommitTrailer(oid))
    }
}

impl TryFrom<Vec<OwnedTrailer>> for ContainingIdentityCommitTrailer {
    type Error = error::InvalidContainingIdentityTrailer;

    fn try_from(trailers: Vec<OwnedTrailer>) -> Result<Self, Self::Error> {
        let trailer_refs = trailers.iter().map(Trailer::from);
        ContainingIdentityCommitTrailer::from_trailers(trailer_refs)
    }
}
