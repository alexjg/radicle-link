...
use node_lib::api::{
    io,
    io::{AsyncTransport as _, Transport as _},
    messages,
};
...
proptest! {
    #[test]
    fn test_request_sync_round_trip(requests in uniform3(request())) {
        with_sync_transport(|mut left, mut right| {
            let mut result = Vec::new();
            for request in &requests {
                left.send_request(request.clone()).unwrap();
            }
            while result.len() < 3 {
                let message = right.recv_request().unwrap();
                result.push(message.unwrap());
            }
            left.close().unwrap();
            assert!(right.recv_request().unwrap().is_none());
            assert_eq!(requests.to_vec(), result);
        })
    }

    #[test]
    fn test_response_sync_round_trip(responses in uniform3(response())) {
        with_sync_transport(|mut left, mut right| {
            let mut result = Vec::new();
            for response in &responses {
                left.send_response(response.clone()).unwrap();
            }
            while result.len() < 3 {
                let message = right.recv_response().unwrap();
                result.push(message.unwrap());
            }
            left.close().unwrap();
            assert!(right.recv_response().unwrap().is_none());
            assert_eq!(responses.to_vec(), result);
        })
    }

    #[test]
    fn test_request_async_round_trip(requests in uniform3(request())) {
    ...
    #[test]
    fn test_response_async_round_trip(responses in uniform3(response())) {
...
type SyncConnection = io::SocketTransport<io::SyncSocketTransport>;

fn with_sync_transport<F: FnOnce(SyncConnection, SyncConnection)>(f: F) {
    let (left, right) = std::os::unix::net::UnixStream::pair().unwrap();
    f(left.into(), right.into())
}

type AsyncConnection = io::SocketTransport<io::TokioSocketTransport>;

fn with_async_transport<
    F: FnOnce(AsyncConnection, AsyncConnection) -> FU,
