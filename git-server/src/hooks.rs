    ...
    >(
        ...
        if self.post_receive.announce {
            ...
            if let Some(rpc_socket_path) = self.rpc_socket_path {
                ...
                let conn = node_lib::api::client::Connection::connect("linkd-git", rpc_socket_path)
                    .await
                    .map_err(|e| Error::LinkdConnect(Box::new(e)))?;
                let cmd = node_lib::api::client::Command::announce(urn.clone(), at.into());
                let mut replies = cmd
