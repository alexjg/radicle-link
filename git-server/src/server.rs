...
impl SshHandler {
    fn channel_id(&self, channel: thrussh::ChannelId) -> ChannelAndSessionId {
        ChannelAndSessionId::new(channel, self.id.clone())
    }
}
...
impl thrussh::server::Handler for SshHandler {
    ...
    fn exec_request(
        ...
        let (service, urn) = match exec_str::parse_exec_str(&exec_str) {
            Ok(s) => s,
            Err(e) => {
        ...
        tracing::debug!(%service, %urn, "parsed exec_request");
        ...
        async move {
            match self
                .handle
                .exec_git(id, handle, service, urn, self.hooks.clone())
                .await
            {
                Ok(_) => {
